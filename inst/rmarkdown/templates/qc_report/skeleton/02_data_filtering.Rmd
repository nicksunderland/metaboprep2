...
## Exclusion summary
```{r exclusion_table}
excl <- metabolites@exclusions[["qcing"]]

excl_tbl <- rbind(
  data.table::data.table(Reason = c("Features", paste0("   - ", gsub("_", " ", names(excl[["features"]])))), 
                         Count  = c("", as.character(sapply(excl[["features"]], length)))),
  data.table::data.table(Reason = c("Samples", paste0("   - ", gsub("_", " ", names(excl[["samples"]])))), 
                         Count  = c("", as.character(sapply(excl[["samples"]], length))))
)
...
sub_notes <- c(
  ...
  glue::glue("Sample exclusions based on the user defined threshold of *>={sprintf('%.0f%%', metabolites@sample_missingness*100)}."),
  "Features with missingness >=80% (xenobiotics are not included in this step).",
  glue::glue("Feature exclusions based on user defined threshold of *>={sprintf('%.0f%%', metabolites@feature_missingness*100)}",
             "(xenobiotics are not included in this step)."),
  glue::glue("Samples with a total-peak-area or total-sum-abundance that is *>={metabolites@total_peak_area_sd} SD from the mean."),
  glue::glue("Samples that are *>={metabolites@outlier_udist} SD from the mean on principal component axis 1 and 2")
