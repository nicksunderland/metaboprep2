...
#' @export
generate_report <- new_generic("generate_report", c("metabolites"), function(metabolites, output_dir, output_filename=NULL, format="pdf", template="qc_report") { S7_dispatch() })
...
method(generate_report, Metabolites) <- function(metabolites, output_dir, output_filename=NULL, format="pdf", template="qc_report") {
  ...
  if (is.null(output_filename)) {
    outpath <- file.path(output_dir, clean_names(paste0(metabolites@project_name, "_metaboprep2_", template)))
  } else {
  ...
  rmarkdown::render(
    input         = template_path,
    output_file   = outpath,
    params        = list(metabolites = metabolites),
