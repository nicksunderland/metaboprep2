...
method(metabolite_qc, Metabolites) <- function(metabolites, source="raw"){
  ...
  # run normalisation
  if (any(grepl("(?i)platform", names(get_features(metabolites, layer=source))))) {
  ...
  incl_feats    <- setdiff(incl_feats, unlist(metabolites@exclusions[["qcing"]][["features"]]))
  if (metabolites@derived_var_exclusion) {
    ...
    incl_feats    <- setdiff(incl_feats, derived_feats)
  } else {
  ...
  if (metabolites@xenobiotics_var_exclusion) {
    ...
    incl_feats       <- setdiff(incl_feats, xenobiotic_feats)
  } else {
  ...
  metabolites <- feature_summary(metabolites, layer = "qcing", sample_ids=incl_samps, feature_ids=incl_feats)

  # identify PC outliers using the newly populated @feature_summary data
  metabolites <- pc_and_outliers(metabolites, layer = "qcing", sample_ids=incl_samps, feature_ids=incl_feats)
  ...
  incl_feats  <- unique(c(incl_feats, xenobiotic_feats)) # add back
  dat <- get_data(metabolites, layer="qcing", sample_ids=incl_samps, feature_ids=incl_feats)
  metabolites@data <- add_layer(current    = metabolites@data,
                                layer      = dat,
                                layer_name = "post_qc",
  ...
  metabolites <- sample_summary( metabolites, layer="post_qc", sample_ids=setdiff(incl_samps,xenobiotic_feats),
